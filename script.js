...
document.addEventListener('DOMContentLoaded', function(){
  ...
  (function injectSprite(){
    ...
    fetch('icons.svg').then(r=>{ if(r.ok) return r.text(); throw new Error('no sprite'); }).then(text=>{
  ...
  function applyTheme(t){ document.body.setAttribute('data-theme', t); toggle.setAttribute('aria-pressed', t==='dark'); if(logoImg){ if(t==='dark'){ logoImg.setAttribute('src','media/blanc.png') } else { logoImg.setAttribute('src','media/noir.png') } } if(iconMoon && iconSun){ if(t==='dark'){ iconMoon.style.display='none'; iconSun.style.display='block'; } else { iconMoon.style.display='block'; iconSun.style.display='none'; } } }
    applyTheme(initial);
